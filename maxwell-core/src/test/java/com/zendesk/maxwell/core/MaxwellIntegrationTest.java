...
public class MaxwellIntegrationTest extends MaxwellTestWithIsolatedServer {
	...
	public void testJdbcConnectionOptions() throws Exception {
		...
		Set<String> maxwellMysqlParams = new HashSet<>(Arrays.asList(config.maxwellMysql.getConnectionURI().split("\\?")[1].split("&")));
		...
		Set<String> replicationMysqlParams = new HashSet<>(Arrays.asList(config.replicationMysql.getConnectionURI().split("\\?")[1].split("&")));
