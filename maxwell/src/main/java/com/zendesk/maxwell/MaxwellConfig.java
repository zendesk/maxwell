...
public class MaxwellConfig extends AbstractConfig {
	...
	protected OptionParser buildOptionParser() {
		...
		parser.accepts("secret_key", "The secert key for the AES encryption").withOptionalArg();
		parser.accepts("use_encryption", "boolean flag to enable encryption").withOptionalArg();
		parser.accepts("trim_string", "Interger to trim string to this size").withOptionalArg();
		parser.accepts("remove_nonascii", "boolean flag to enable removal of all non-ascii characters").withOptionalArg();
		parser.accepts("include_nulls", "boolean flag to enable including nulls in record").withOptionalArg();
	...
	private void parse(String [] argv) {
		...
		// Dave Added for Encryption
		if ( options.has("use_encryption"))
			this.useEncryption = Boolean.valueOf((String) options.valueOf("use_encryption"));

		if ( options.has("encryption_key"))
			this.encryptionKey = (String) options.valueOf("encryption_key");

		if ( options.has("secret_key"))
			this.secertKey = (String) options.valueOf("secret_key");

		if ( options.has("trim_string"))
			this.trimString = Integer.valueOf((String) options.valueOf("trim_string"));

		if ( options.has("remove_nonascii"))
			this.removeNonAscii = Boolean.valueOf((String) options.valueOf("remove_nonascii"));

		if ( options.has("include_nulls"))
			this.includeNulls = Boolean.valueOf((String) options.valueOf("include_nulls"));
	...
	private void parseFile(String filename, Boolean abortOnMissing) {
		...
		// Dave Added for Encryption
		this.encryptionKey = p.getProperty("encryption_key");
		this.useEncryption = Boolean.valueOf(p.getProperty("use_encryption"));
		this.secertKey = p.getProperty("secret_key");
		this.removeNonAscii = Boolean.valueOf(p.getProperty("remove_nonascii"));
		this.trimString = Integer.valueOf(p.getProperty("trim_string", "0"));
		this.includeNulls = Boolean.valueOf(p.getProperty("include_nulls"));
