...
public class SchemaCapturer {
	...
	private static String[] extractEnumValues(String expandedType) {
		...
		for(int j=0 ; j < enumValues.length; j++) {
			...
			// Dave added if block to add the string null if enum is an empty string
			// Originally it was offsetting the enum stored and throwing an error.
			if ( enumValues[j].length() == 0 )
				enumValues[j] = "null";
		}
		return enumValues;
	}
